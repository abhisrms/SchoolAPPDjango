...
def afterlogin_view(request):
    if is_admin(request.user):
        ...
        # Handle the case where the user doesn't fall into any specified role
        return HttpResponse("You don't have the required role.")    
...
class CustomLogoutView(LogoutView):
    def dispatch(self, request, *args, **kwargs):
        # If it's a GET request, log the user out and redirect
        if request.method == 'GET' and request.user.is_authenticated:
            # Call the parent class's dispatch method, which handles the logout
            response = super().dispatch(request, *args, **kwargs)
            # Your custom logout code here (if needed)
            return response

        # If it's a POST request or the user is not authenticated, proceed with the default behavior
        return super().dispatch(request, *args, **kwargs)
    

def get_fee(request, cl):
        try:
            fee = Fee.objects.get(cl=cl).amount
            return JsonResponse({'fee': fee})
        except Fee.DoesNotExist:
            return JsonResponse({'fee': 0})
        

def update_fee_view(request):
    forms = []

    if request.method == 'POST':
        forms = [FeeUpdateForm(request.POST, prefix=str(fee.id), instance=fee) for fee in Fee.objects.exclude(cl=None)]
        if all(form.is_valid() for form in forms):
            for form in forms:
                form.save()
        messages.success(request, "Fees updated successfully.")
        return redirect('admin-update-class-fee')
    else:
        fees = Fee.objects.exclude(cl=None)
        forms = [FeeUpdateForm(prefix=str(fee.id), instance=fee) for fee in fees]
...
def get_students_by_class(request):
    ...
    if cl:
        students_query = StudentExtra.objects.filter(cl=cl)
        students = [{'id': student.get_id, 'name': student.get_name} for student in students_query]
...
def student_fee_details(request):
    if request.method == 'GET':
        ...
        return render(request, 'school/student_fee_details.html', {'students': students})
...
def class_fee_select(request):
    return render(request, 'school/admin_class_fee_select.html')
...
def class_fee_report(request):
    ...
    last_day_of_month = (selected_date + timedelta(days=32)).replace(day=1) - timedelta(days=1)
    ...
    for cl in classes:
        ...
        total_fee = total_students * total_fee_per_student
